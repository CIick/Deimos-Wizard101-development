...
from src.SlackPack import SlackPack
...
class Quester():
    ...
    async def handle_normal_quests(self, follower_clients: list[Client], questing_friend_tp: bool):
        ...
        if await is_free_leader_questing(self.current_leader_client):
            ...
            if await is_visible_by_path(self.current_leader_client, npc_range_path) and calc_Distance(leader_client_objective_xyz, current_pos) < 750.0:
                ...
                if 'Go To Upper Room in Caretaker' in quest_objective:
                    await asyncio.gather(navmap_tp(self.client, pet_mode=True))
                    print('dasdsadsadada')
    ...
    async def auto_quest(self, ignore_pet_level_up: bool, play_dance_game: bool):
        self.starting_client_level = await self.client.stats.reference_level()
        while self.client.questing_status:
            await asyncio.sleep(1)
            # Each time we TP to a quest we check if the user leveled up.
            client_level = await self.client.stats.reference_level()
            bone_zone = await self.client.zone_name()
            print('Start Level', self.starting_client_level, 'Current Level', client_level, 'Current Zone', bone_zone)
            if client_level > self.starting_client_level:
                each_client = SlackPack(self.client)
                await each_client.open_and_parse_backpack_contents(self.client)
                self.starting_client_level = client_level
                print('Start Level (Changed after level Up):', self.starting_client_level, 'Current Level:', client_level)
